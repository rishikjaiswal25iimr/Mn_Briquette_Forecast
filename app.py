"""
Mn Briquette Hybrid Forecasting — Streamlit Dashboard  ·  v4.3
=============================================================
...
def _add_confidence_band(fig: go.Figure, dates, values: np.ndarray):
    n = len(values)
    idx = np.arange(1, n+1)
...
def chart_price_comparison(hist: pd.DataFrame, future: pd.DataFrame, display_window: int, price_mode: str, show_regime: bool, show_market: bool) -> go.Figure:
    ...
    if not future.empty and future_col in future.columns:
        ...
        # Calculate upper and lower bounds for custom hover data
        upper, lower = _add_confidence_band(fig, future.index, fp.values)
        ...
        fig.add_trace(go.Scatter(
            x=future.index, y=fp, name="Future Forecast Price", mode="lines", line=dict(color=C_FUTURE, width=3),
...
def chart_dual_axis(hist: pd.DataFrame, display_window: int) -> go.Figure:
    """ Dual Axis chart now strictly compares Historical Index vs Historical Price ONLY """
    cutoff = hist.index[-1] - pd.DateOffset(weeks=display_window)
    h = hist[hist.index >= cutoff].copy()
    ...
    # Corrected Chart Title and removed future lines completely
...
def render_steel_calculator(hist: pd.DataFrame, future: pd.DataFrame, display_window: int):
    ...
    if "real_price" in fut.columns:
        fig.add_trace(go.Scatter(x=fut.index, y=fut["real_price"], name="Future Forecast Price", mode="lines", line=dict(color=C_FUTURE, width=3), hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Future Forecast Price: ₹%{y:.2f}<extra></extra>"))
...
def main():
    ...
    <div style='padding:12px 0 6px;'>
      <h1 style='font-size:25px;margin:0;color:#111;'>📈 Mn Briquette Price Forecasting &nbsp; <span style='font-size:14px;color:#666;'>v4.3</span></h1>
    ...
    with tab1:
        ...
        if show_dual: 
            # Note: Removed `future` argument to cleanly stop Dual Axis chart at present day
            st.plotly_chart(chart_dual_axis(hist, display_window), use_container_width=True)
