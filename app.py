"""
Mn Briquette Hybrid Forecasting — Streamlit Dashboard  ·  v4.4
=============================================================
...
def chart_price_comparison(hist: pd.DataFrame, future: pd.DataFrame, display_window: int, price_mode: str, show_regime: bool, show_market: bool) -> go.Figure:
    ...
    # 2. FUTURE TRACES (With CI and Seamless Connection)
    if not future.empty and future_col in future.columns:
        fp = future[future_col]
        
        # Connect visual line to last historical point
        conn_x = [h.index[-1]] + list(future.index)
        conn_y = [actual_vals.iloc[-1]] + list(fp.values)
        
        # Calculate Expanding Confidence Interval (Starts at 0 at the historical boundary)
        n = len(fp)
        idx = np.arange(1, n+1)
        sigma = np.std(fp.values) * 0.015 * idx
        conn_upper = [actual_vals.iloc[-1]] + list(fp.values + 1.96 * sigma)
        conn_lower = [actual_vals.iloc[-1]] + list(fp.values - 1.96 * sigma)
        
        # Draw CI Band (No hover to keep tooltip clean)
        fig.add_trace(go.Scatter(x=conn_x + conn_x[::-1], y=conn_upper + conn_lower[::-1], fill="toself", fillcolor=C_CI, line=dict(width=0), name="95% CI", showlegend=True, hoverinfo="skip"))
        
        # Format the exact requested hover logic using customdata
        custom_data = np.stack((conn_upper, conn_lower), axis=-1)
        ...
        fig.add_trace(go.Scatter(
            x=conn_x, y=conn_y, name="Future Forecast Price", mode="lines", line=dict(color=C_FUTURE, width=3),
...
def chart_dual_axis(hist: pd.DataFrame, future: pd.DataFrame, display_window: int) -> go.Figure:
    ...
    # 1. HISTORICAL TRACES
    fig.add_trace(go.Scatter(x=h.index, y=h.get("hybrid_prediction", h["lgb_prediction"]), name="Index", mode="lines", line=dict(color=C_HYBRID, width=3), hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Index: %{y:.2f}<extra></extra>"), secondary_y=False)
    ...
    # 2. FUTURE TRACES (Visually Connected)
    if not future.empty:
        conn_x = [h.index[-1]] + list(future.index)
        
        if "predicted_index" in future.columns:
            conn_y_idx = [h.get("hybrid_prediction", h["lgb_prediction"]).iloc[-1]] + list(future["predicted_index"])
            fig.add_trace(go.Scatter(x=conn_x, y=conn_y_idx, name="Future Index", mode="lines", line=dict(color="#64B5F6", width=3), hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Future Index: %{y:.2f}<extra></extra>"), secondary_y=False)
            
        if "real_price" in future.columns:
            conn_y_px = [h["real_price"].iloc[-1]] + list(future["real_price"])
            fig.add_trace(go.Scatter(x=conn_x, y=conn_y_px, name="Future Index Price", mode="lines", line=dict(color="#FF5722", width=2.5, dash="dash"), hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Future Index Price: ₹%{y:.2f}<extra></extra>"), secondary_y=True)
...
def render_steel_calculator(hist: pd.DataFrame, future: pd.DataFrame, display_window: int):
    ...
    # Future Connected
    if not future.empty and "real_price" in fut.columns:
...
def main():
    ...
    <div style='padding:12px 0 6px;'>
      <h1 style='font-size:25px;margin:0;color:#111;'>📈 Mn Briquette Price Forecasting &nbsp; <span style='font-size:14px;color:#666;'>v4.4</span></h1>
