"""
Mn Briquette Hybrid Forecasting — Streamlit Dashboard  ·  v4.1
=============================================================
...
C_MARKET   = "#9C27B0" # Deep Magenta/Purple for Excel Market
...
def chart_price_comparison(hist: pd.DataFrame, future: pd.DataFrame, display_window: int, price_mode: str, show_regime: bool, show_market: bool) -> go.Figure:
    ...
    if price_mode == "real":
        actual_vals, hybrid_vals = h.get("real_price", h["actual"]), h.get("real_price", h.get("hybrid_prediction", h["lgb_prediction"]))
        y_title, future_col = "Price (Rs/Kg)", "real_price"
    else:
        actual_vals, hybrid_vals = h["actual"], h.get("hybrid_prediction", h["lgb_prediction"])
    ...
    fig.add_trace(go.Scatter(x=h.index, y=actual_vals, name="Model Base", mode="lines", line=dict(color=C_HYBRID, width=3)))
    
    if show_market and "market_price" in h.columns:
        fig.add_trace(go.Scatter(x=h.index, y=h["market_price"], name="Excel Market Price", mode="lines", line=dict(color=C_MARKET, width=2.5)))

    if not future.empty and future_col in future.columns:
        fig.add_trace(go.Scatter(x=future.index, y=future[future_col], name="Future Forecast", mode="lines", line=dict(color=C_FUTURE, width=3)))
...
def chart_dual_axis(hist: pd.DataFrame, future: pd.DataFrame, display_window: int) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(x=h.index, y=h.get("hybrid_prediction", h["lgb_prediction"]), name="Historical Index", mode="lines", line=dict(color=C_HYBRID, width=3)), secondary_y=False)
    if "real_price" in h.columns:
        fig.add_trace(go.Scatter(x=h.index, y=h["real_price"], name="Historical Price", mode="lines", line=dict(color=C_ACTUAL, width=2.5, dash="dash")), secondary_y=True)

    if not future.empty:
        if "predicted_index" in future.columns:
            fig.add_trace(go.Scatter(x=future.index, y=future["predicted_index"], name="Future Index", mode="lines", line=dict(color="#64B5F6", width=3)), secondary_y=False)
        if "real_price" in future.columns:
            fig.add_trace(go.Scatter(x=future.index, y=future["real_price"], name="Future Price", mode="lines", line=dict(color="#FF5722", width=2.5, dash="dash")), secondary_y=True)
...
def chart_feature_donut(fi: pd.DataFrame) -> go.Figure:
    ...
    fig = go.Figure(data=[go.Pie(labels=top["feature"], values=top["percent"], hole=0.5, textinfo='label+percent', marker=dict(colors=["#1E88E5", "#43A047", "#E53935", "#FB8C00", "#8E24AA", "#00ACC1", "#7CB342", "#9E9E9E"]))])
...
# ═════════════════════════════════════════════════════════════════════════════
# STEEL VALUE CALCULATOR (Restored Tab 4)
# ═════════════════════════════════════════════════════════════════════════════
...
def render_steel_calculator(hist: pd.DataFrame, future: pd.DataFrame, display_window: int):
    ...
    fig = go.Figure()
    if "real_price" in h.columns:
        fig.add_trace(go.Scatter(x=h.index, y=h["real_price"], name="Index Price (Rs/Kg)", mode="lines", line=dict(color=C_HYBRID, width=3)))
    if "real_price" in fut.columns:
        fig.add_trace(go.Scatter(x=fut.index, y=fut["real_price"], name="Future Index Price", mode="lines", line=dict(color=C_FUTURE, width=3)))
    if not h["mn_value"].isna().all():
        fig.add_trace(go.Scatter(x=h.index, y=h["mn_value"], name="Mn Value (Rs/Kg Mn)", mode="lines", line=dict(color=C_ACTUAL, width=2.5, dash="dash"), yaxis="y2"))
    if not fut["mn_value"].isna().all():
        fig.add_trace(go.Scatter(x=fut.index, y=fut["mn_value"], name="Future Mn Value", mode="lines", line=dict(color="#FF5722", width=2.5, dash="dash"), yaxis="y2"))
...
def main():
    ...
    <div style='padding:12px 0 6px;'>
      <h1 style='font-size:25px;margin:0;color:#111;'>📈 Mn Briquette Price Forecasting &nbsp; <span style='font-size:14px;color:#666;'>v4.1</span></h1>
      <p style='color:#555;font-size:13px;margin:3px 0 0;'>Structural Break Modeling | Rolling Dynamics | Steel Value Optimizer</p>
    </div>
    ...
    # Main Top KPIs (Restored Missing KPIs)
    ...
    c2.metric("End Forecast", f"₹{end_real:.2f}", delta=f"{len(future)} wks ahead", delta_color="off")
    c3.metric("In-Sample MAPE", f"{mape:.2f}%")
    c4.metric("Scaling Factor", f"{cal.get('scaling_factor', 0):.4f}", delta="1 idx = X Rs/Kg", delta_color="off")
    if "market_price" in hist.columns:
        last_mkt = hist["market_price"].dropna()
        if not last_mkt.empty: c5.metric("Excel Market Price", f"₹{last_mkt.iloc[-1]:.2f}")
