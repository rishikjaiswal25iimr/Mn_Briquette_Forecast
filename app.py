"""
Mn Briquette Hybrid Forecasting — Streamlit Dashboard  ·  v4.5
=============================================================
...
def chart_price_comparison(hist: pd.DataFrame, future: pd.DataFrame, display_window: int, price_mode: str, show_regime: bool, show_market: bool) -> go.Figure:
    ...
    # 1. ESTABLISH SHARED X-AXIS FOR STRICT HOVER ISOLATION
    full_x = list(h.index) + list(future.index)
    N = len(h)
    M = len(future)

    # 2. HISTORICAL TRACES (Padded with None so they disappear in the future)
    hist_actual_y = list(actual_vals) + [None] * M
    fig.add_trace(go.Scatter(
        x=full_x, y=hist_actual_y, name="Index Price", mode="lines", line=dict(color=C_HYBRID, width=3),
        hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Index Price: " + fmt + "<extra></extra>"
    ))
    
    if show_market and "market_price" in h.columns:
        hist_market_y = list(h["market_price"]) + [None] * M
        fig.add_trace(go.Scatter(
            x=full_x, y=hist_market_y, name="Market Price", mode="lines", line=dict(color=C_MARKET, width=2.5),
            hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Market Price: " + fmt + "<extra></extra>"
        ))

    # 3. FUTURE TRACES (Padded with None so they disappear in the past)
    if not future.empty and future_col in future.columns:
        fp = future[future_col]
        conn_val = actual_vals.iloc[-1]
        
        # Y values start exactly at the connection point
        fut_y = [None]*(N-1) + [conn_val] + list(fp.values)
        
        # Calculate Expanding Confidence Interval
        idx = np.arange(1, M+1)
        sigma = np.std(fp.values) * 0.015 * idx
        ci_up_fut = fp.values + 1.96 * sigma
        ci_dn_fut = fp.values - 1.96 * sigma
        
        # Draw CI Band visually 
        ci_x = [h.index[-1]] + list(future.index)
        ci_upper_fill = [conn_val] + list(ci_up_fut)
        ci_lower_fill = [conn_val] + list(ci_dn_fut)
        fig.add_trace(go.Scatter(
        ...
        # Customdata arrays for exact max/min hover tooltips
        upper = [None]*(N-1) + [conn_val] + list(ci_up_fut)
        lower = [None]*(N-1) + [conn_val] + list(ci_dn_fut)
        custom_data = np.stack((upper, lower), axis=-1)
        
        fmt_up = fmt.replace("%{y", "%{customdata[0]")
        fmt_dn = fmt.replace("%{y", "%{customdata[1]")
        
        htemplate = (
            "<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>"
            "Future Forecast Price: " + fmt + "<br>"
            "Max (95% CI): " + fmt_up + "<br>"
            "Min (95% CI): " + fmt_dn + "<extra></extra>"
        )

        fig.add_trace(go.Scatter(
            x=full_x, y=fut_y, name="Future Forecast Price", mode="lines", 
            line=dict(color=C_FUTURE, width=3), customdata=custom_data, hovertemplate=htemplate
        ))
...
def chart_dual_axis(hist: pd.DataFrame, future: pd.DataFrame, display_window: int) -> go.Figure:
    ...
    # 1. HISTORICAL TRACES (Isolate hover to past)
    h_idx_vals = h.get("hybrid_prediction", h["lgb_prediction"])
    hist_idx_y = list(h_idx_vals) + [None] * M
    fig.add_trace(go.Scatter(x=full_x, y=hist_idx_y, name="Index", mode="lines", line=dict(color=C_HYBRID, width=3), hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Index: %{y:.2f}<extra></extra>"), secondary_y=False)
    
    if "real_price" in h.columns:
        hist_rp_y = list(h["real_price"]) + [None] * M
        fig.add_trace(go.Scatter(x=full_x, y=hist_rp_y, name="Index Price", mode="lines", line=dict(color=C_ACTUAL, width=2.5, dash="dash"), hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Index Price: ₹%{y:.2f}<extra></extra>"), secondary_y=True)

    # 2. FUTURE TRACES (Isolate hover to future)
    if not future.empty:
        if "predicted_index" in future.columns:
            fut_idx_conn = h_idx_vals.iloc[-1]
            fut_idx_y = [None]*(N-1) + [fut_idx_conn] + list(future["predicted_index"])
            fig.add_trace(go.Scatter(x=full_x, y=fut_idx_y, name="Future Index", mode="lines", line=dict(color="#64B5F6", width=3), hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Future Index: %{y:.2f}<extra></extra>"), secondary_y=False)
            
        if "real_price" in future.columns:
            fut_rp_conn = h["real_price"].iloc[-1]
            fut_rp_y = [None]*(N-1) + [fut_rp_conn] + list(future["real_price"])
            fig.add_trace(go.Scatter(x=full_x, y=fut_rp_y, name="Future Index Price", mode="lines", line=dict(color="#FF5722", width=2.5, dash="dash"), hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Future Index Price: ₹%{y:.2f}<extra></extra>"), secondary_y=True)

    fig.update_layout(**_layout("Index vs Price — Dual Axis Comparison", "Price Index", 400))
...
def render_steel_calculator(hist: pd.DataFrame, future: pd.DataFrame, display_window: int):
    ...
    full_x = list(h.index) + list(fut.index)
    N = len(h)
    M = len(fut)

    # 1. HISTORICAL TRACES (Isolated to Past)
    if "real_price" in h.columns:
        hist_rp_y = list(h["real_price"]) + [None] * M
        fig.add_trace(go.Scatter(x=full_x, y=hist_rp_y, name="Index Price (Rs/Kg)", mode="lines", line=dict(color=C_HYBRID, width=3), hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Index Price: ₹%{y:.2f}<extra></extra>"))
    if not h["mn_value"].isna().all():
        hist_mn_y = list(h["mn_value"]) + [None] * M
        fig.add_trace(go.Scatter(x=full_x, y=hist_mn_y, name="Mn Value (Rs/Kg Mn)", mode="lines", line=dict(color=C_ACTUAL, width=2.5, dash="dash"), yaxis="y2", hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Mn Value: ₹%{y:.2f}<extra></extra>"))
    
    # 2. FUTURE TRACES (Isolated to Future)
    if not fut.empty and "real_price" in fut.columns:
        rp_conn = h["real_price"].iloc[-1]
        mn_conn = h["mn_value"].iloc[-1]
        
        fut_rp_y = [None]*(N-1) + [rp_conn] + list(fut["real_price"])
        fut_mn_y = [None]*(N-1) + [mn_conn] + list(fut["mn_value"])
        
        fig.add_trace(go.Scatter(x=full_x, y=fut_rp_y, name="Future Forecast Price", mode="lines", line=dict(color=C_FUTURE, width=3), hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Future Forecast Price: ₹%{y:.2f}<extra></extra>"))
        fig.add_trace(go.Scatter(x=full_x, y=fut_mn_y, name="Future Mn Value", mode="lines", line=dict(color="#FF5722", width=2.5, dash="dash"), yaxis="y2", hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Future Mn Value: ₹%{y:.2f}<extra></extra>"))

        fig.add_vline(x=h.index[-1].timestamp()*1000, line=dict(color="rgba(0,0,0,0.3)", width=1, dash="dot"), annotation_text="Today")
...
def main():
    ...
    <div style='padding:12px 0 6px;'>
      <h1 style='font-size:25px;margin:0;color:#111;'>📈 Mn Briquette Price Forecasting &nbsp; <span style='font-size:14px;color:#666;'>v4.5</span></h1>
