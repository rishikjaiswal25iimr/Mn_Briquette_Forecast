"""
Mn Briquette Hybrid Forecasting — Streamlit Dashboard  ·  v4.6
=============================================================
...
# ─────────────────────────────────────────────────────────────────────────────
# CHART BUILDERS (STRICT HOVER ISOLATION APPLIED)
# ─────────────────────────────────────────────────────────────────────────────
...
def chart_price_comparison(hist: pd.DataFrame, future: pd.DataFrame, display_window: int, price_mode: str, show_regime: bool, show_market: bool) -> go.Figure:
    ...
    # 1. HISTORICAL TRACES (Isolated purely to historical dates)
    fig.add_trace(go.Scatter(
        x=h.index, y=actual_vals, name="Index Price", mode="lines", line=dict(color=C_HYBRID, width=3),
        hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Index Price: " + fmt + "<extra></extra>"
    ))
    
    if show_market and "market_price" in h.columns:
        fig.add_trace(go.Scatter(
            x=h.index, y=h["market_price"], name="Market Price", mode="lines", line=dict(color=C_MARKET, width=2.5),
            hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Market Price: " + fmt + "<extra></extra>"
        ))

    # 2. FUTURE TRACES (Isolated purely to future dates + 1 connector point)
    if not future.empty and future_col in future.columns:
        fp = future[future_col]
        conn_val = actual_vals.iloc[-1]
        
        # Exact arrays for future trace only
        conn_x = [h.index[-1]] + list(future.index)
        conn_y = [conn_val] + list(fp.values)
        
        # Calculate Expanding Confidence Interval
        idx = np.arange(1, len(fp) + 1)
        sigma = np.std(fp.values) * 0.015 * idx
        ci_up_fut = list(fp.values + 1.96 * sigma)
        ci_dn_fut = list(fp.values - 1.96 * sigma)
        
        # Draw CI Band visually 
        ci_upper_fill = [conn_val] + ci_up_fut
        ci_lower_fill = [conn_val] + ci_dn_fut
        fig.add_trace(go.Scatter(
            x=conn_x + conn_x[::-1], y=ci_upper_fill + ci_lower_fill[::-1], 
        ...
        custom_data = np.stack((ci_upper_fill, ci_lower_fill), axis=-1)
        ...
        fig.add_trace(go.Scatter(
            x=conn_x, y=conn_y, name="Future Forecast Price", mode="lines", 
...
def chart_dual_axis(hist: pd.DataFrame, future: pd.DataFrame, display_window: int) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(x=h.index, y=h_idx_vals, name="Index", mode="lines", line=dict(color=C_HYBRID, width=3), hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Index: %{y:.2f}<extra></extra>"), secondary_y=False)
    
    if "real_price" in h.columns:
        fig.add_trace(go.Scatter(x=h.index, y=h["real_price"], name="Index Price", mode="lines", line=dict(color=C_ACTUAL, width=2.5, dash="dash"), hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Index Price: ₹%{y:.2f}<extra></extra>"), secondary_y=True)

    # 2. FUTURE TRACES (Isolated to Future)
    if not future.empty:
        conn_x = [h.index[-1]] + list(future.index)
        if "predicted_index" in future.columns:
            fut_idx_y = [h_idx_vals.iloc[-1]] + list(future["predicted_index"])
            fig.add_trace(go.Scatter(x=conn_x, y=fut_idx_y, name="Future Index", mode="lines", line=dict(color="#64B5F6", width=3), hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Future Index: %{y:.2f}<extra></extra>"), secondary_y=False)
            
        if "real_price" in future.columns:
            fut_rp_y = [h["real_price"].iloc[-1]] + list(future["real_price"])
            fig.add_trace(go.Scatter(x=conn_x, y=fut_rp_y, name="Future Index Price", mode="lines", line=dict(color="#FF5722", width=2.5, dash="dash"), hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Future Index Price: ₹%{y:.2f}<extra></extra>"), secondary_y=True)
...
def render_steel_calculator(hist: pd.DataFrame, future: pd.DataFrame, display_window: int):
    ...
    if not fut.empty and "real_price" in fut.columns:
        conn_x = [h.index[-1]] + list(fut.index)
        conn_y_rp = [h["real_price"].iloc[-1]] + list(fut["real_price"])
        conn_y_mn = [h["mn_value"].iloc[-1]] + list(fut["mn_value"])
        
        fig.add_trace(go.Scatter(x=conn_x, y=conn_y_rp, name="Future Forecast Price", mode="lines", line=dict(color=C_FUTURE, width=3), hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Future Forecast Price: ₹%{y:.2f}<extra></extra>"))
        fig.add_trace(go.Scatter(x=conn_x, y=conn_y_mn, name="Future Mn Value", mode="lines", line=dict(color="#FF5722", width=2.5, dash="dash"), yaxis="y2", hovertemplate="<b>%{x|%B %Y}</b><br>%{x|%Y-%m-%d}<br>Future Mn Value: ₹%{y:.2f}<extra></extra>"))
...
def main():
    ...
    <div style='padding:12px 0 6px;'>
      <h1 style='font-size:25px;margin:0;color:#111;'>📈 Mn Briquette Price Forecasting &nbsp; <span style='font-size:14px;color:#666;'>v4.6</span></h1>
